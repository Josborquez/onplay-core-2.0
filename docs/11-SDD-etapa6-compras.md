...
| C12b | Costos de importación desglosados (§6.7): DIN leída del PDF o digitada (FOB, flete, seguro presunto, arancel, dólar aduanero), gastos del agente y del courier con su documento (`CompraGasto`), IVA de importación y de los servicios **aparte como crédito fiscal**, tarjetas costo puesto en la tienda / IVA a recuperar / desembolso / % sobre FOB — fundamento en `docs/13-importaciones-costo-chile.md` | P0 | 1 (hecho 2026-09-11, R-031; el dueño confirmó que el contador recupera el IVA) |
...
**`Proveedor`** — `id`, `nombre`, `rut` (único, normalizado `91144000-8`, nullable), `lector` (enum `LectorFactura`: `manual` · `andina` · `nico` · `nico_factura` · `coqui` · `devir` · `asmodee` · `blackfaerie`), `activo`, `notas`, `creadoEn`. Con RUT, `POST /compras/leer` reconoce al proveedor solo.
...
**`CompraGasto`** (C12b) — gastos de una importación con su documento: `compraId`, `tipo` (enum `TipoGastoImportacion`: `agente` · `courier` · `seguro` · `otro`), `descripcion`, `montoNeto` (CLP: **costo**, se reparte entre las líneas), `iva` (CLP: crédito fiscal, se reparte en `impuestos`), `documento`, `fecha`.
...
- **D-E6-1 — Costo unitario = total de la línea / unidades, CON IVA e impuestos específicos.** Es lo que cuesta cada unidad puesta en la tienda y lo que se compara con el precio de venta (que también lleva IVA). Se guarda además el `neto` por línea para que la Fase 3 pueda calcular margen neto si el contador lo pide. **Precisión C12b (2026-09-11):** en una importación `neto` de la línea = costo puesto en la tienda (mercancía y flete al tipo de cambio + arancel + gastos netos del agente y del courier) e `impuestos` = la parte de IVA **recuperable** (IVA de importación + IVA de los servicios); `total = neto + impuestos` sigue siendo la base del costo unitario, así el margen se compara igual que en una factura chilena. El seguro presunto (2 % del FOB) es solo base de impuestos: no se paga a nadie y no es costo.
...
**Coqui Hobby Distribution** (`lectores/coqui.ts`, Sanford FL, sin RUT; se reconoce por «Coqui Hobby»; proveedor por `familia`). «Sales Order» en USD: pdfjs entrega una palabra por celda, así que el lector trabaja sobre la fila unida; línea = `N CODIGO: descripción UOM QTY MSRP NET EXT` y las filas siguientes sin ese patrón continúan la descripción; «Order No.», «Order Date» m/d/aaaa, «Currency», «Total (USD)». Cantidades = unidades vendibles (`unidadesPorBulto` 1: «(100 ct.)» son fundas, no unidades); un «*DISPLAY*» se fija al vincular. Kits a 0.00 entran con costo 0 y aviso. **Coqui también manda un «INVOICE»** (referencia 097440, 2026-09-11): celdas enteras `N · (CODIGO) descripción · QTY · UOM · MSRP · PRICE · EXT PRICE`, continuación en celda sola («(24ct)»), «Reference No.», «Date: 01-Sep-2026» (mes en inglés) y una línea «Shipping & Handling» en USD que se reparte entre las líneas con monto según su precio (resto en la última, centavo a centavo) para que Σ `totalOriginal` = «Total (USD)». El mismo lector `coqui` reconoce los dos formatos. Fixtures reales de la orden 0092856 (37 líneas, US$ 1.398,90) y del invoice 097440 (9 líneas, US$ 685,21 con envío US$ 156,71) en `coqui.test.ts`. Migración `e6_moneda_coqui`.
...
- `calcularImportacion({fob, flete, seguro?, arancelPct?, tipoCambioAduana})` → CIF = FOB + flete + seguro (presunto **2 % del FOB** si no viene o si es exactamente ese 2 %, como lo escribe la DIN sin póliza); ad valorem = `arancelPct` % del CIF (6 por defecto, 0 con certificado de origen); IVA = 19 % de (CIF + ad valorem); todo a centavos y a CLP con el dólar aduanero. Test contra la DIN 1150127395-5 (US$ 4.735,16 → CIF 5.148,66, giro US$ 1.345,86 = $1.204.262).
- `convertirLineasAClp(lineas, tipoCambio, costoExtra, ivaExtra)` gana un cuarto parámetro: `costoExtra` = `gastosExtra` + `arancel` + Σ `CompraGasto.montoNeto` va al **neto**; `ivaExtra` = `ivaImportacion` + Σ `CompraGasto.iva` va a **impuestos**; ambos repartidos por `totalOriginal` con el resto en la última línea con monto (los kits a 0 no cargan nada).
- `resumenImportacion(lineas, compra, gastos)` → `costoPuesto` (Σ neto), `ivaRecuperable` (Σ impuestos), `desembolso`, `fobClp`, `sobreFobPct` (cuánto encarece la importación al precio de lista: ≈ +18–19 % en las dos importaciones reales de Coqui), `gastosNetos`, `ivaGastos` y `cuadre` (Σ `totalOriginal` de las líneas contra FOB + flete de la DIN; `difiere` si se alejan más de 1 %).

**Lector de la DIN** (`lectores/din.ts`, `reconoceDin`: «DECLARACION DE INGRESO» + «SERVICIO NACIONAL DE ADUANAS»): la DIN sí trae texto. Lee número (`1150127395-5`), fecha de aceptación (fila `DIA · MES · AÑO` de la hoja de continuación), despachador, consignante, FOB, flete, seguro, CIF, ad valorem (cuenta 223), IVA (cuenta 178), total giro en US$ y en pesos, dólar aduanero, y **un ítem por código del proveedor** («Nombre» del ítem = código de Coqui) con CIF, arancel, IVA, cantidad y FOB unitario; un valor que cae a la fila siguiente («50,88») se recoge igual. `detectarLector` descarta las DIN antes de probar los lectores de factura (la DIN nombra a Coqui y el lector `coqui` la tomaría). Fixture real en `din.test.ts`. Las facturas del agente de aduanas vienen **escaneadas** (sin texto): se digitan como gasto.

**Recalcular:** cualquier cambio de importación o de gastos vuelve a repartir y recalcula `neto/impuestos/total/costoUnitario` de todas las líneas y los totales de la compra. Se permite en **borrador y en recibida** (la DIN y las facturas del agente llegan después de la mercadería): en una recibida se actualiza además `Producto.costoReferencia` de las líneas que ya entraron. Auditado como `editar` sobre la compra con `importacion` / `gasto` anterior y nuevo.
...
**Líneas pendientes de una compra recibida (R-027, pedido del dueño 2026-09-10 tras recibir la orden de Coqui con solo parte vinculada):** las líneas que quedaron fuera (`productoId` null y `movimientoId` null) se pueden vincular después (`PATCH …/lineas/:lineaId`, que en una recibida solo acepta líneas sin movimiento → 409 `LINEA_YA_RECIBIDA` si ya entró) y recibir con el mismo `POST /compras/:id/recibir`: en una recibida entran solo las pendientes con producto, sin tocar lo ya recibido, `recibidaEn` no cambia y la Auditoria lleva `recepcionAdicional:true`; sin pendientes con producto → 409 `SIN_PENDIENTES`. La pantalla avisa al recibir que la compra queda cerrada salvo esas líneas, y en la recibida muestra «N línea(s) quedaron fuera» con Vincular por línea y «Recibir pendientes».
...
- **C12b (§6.7), solo `moneda ≠ CLP` → si no 422 `COMPRA_EN_CLP`; anulada → 409:** `POST /compras/leer-din {archivo}` → `{ din, calculo }` (no escribe; 422 `NO_ES_DIN`) · `PUT /compras/:id/importacion {fob, flete?, seguro?|null, arancelPct?, tipoCambioAduana?, arancel?, ivaImportacion?, dinNumero?, dinFecha?}` o `{quitar:true}` (los CLP girados se calculan si no vienen) → detalle · `POST /compras/:id/gastos {tipo, descripcion, montoNeto, iva?, documento?, fecha?}` → 201 `{gasto, compra}` · `DELETE /compras/:id/gastos/:gastoId` → detalle. `GET /compras/:id` trae `gastos[]` y `resumenImportacion`. `POST /compras/leer` con una DIN → 422 `ES_DIN`.
...
- **V27 · Importación** (C12b, solo compras en moneda extranjera, borrador o recibida): sección con cuatro tarjetas (Costo puesto en la tienda · IVA a recuperar · Desembolso · Sobre el FOB), bloque «Declaración de ingreso (DIN)» con **Subir DIN (PDF)** (→ `POST /compras/leer-din` → diálogo prellenado con lo leído, ítems que coinciden con los códigos de la compra y advertencias) o **Digitar** (FOB, flete, seguro vacío = presunto, arancel %, dólar aduanero, N° y fecha; vista previa CIF / ad valorem / IVA / total giro; los CLP girados se rellenan solos y se pueden corregir), «Quitar DIN» con confirmación; bloque «Gastos de agente y courier» con lista (tipo, documento, neto, IVA) y diálogo «Agregar gasto» (quién lo cobra, descripción sugerida, neto, IVA con el 19 % sugerido, documento, fecha); aviso si las líneas no cuadran con FOB + flete de la DIN. La tarjeta «Total» pasa a llamarse «Desembolso» con «costo · IVA a recuperar». En el paso Moneda de V26 el campo se llama «Gastos sin documento».
...
- **Lector Black Faerie — 2026-09-22** con la factura 2940 (`docs/pdf/N 2940 BLACKFAERIE_CLIENTE_…pdf`): 53 productos en 4 páginas, neto $1.413.613, total $1.682.200, cuadrado sin avisos (R-034).
